...
public class JSqlSplit {

    /**
     * Splits a string containing multiple SQL statements into a vector of SQL
     * statements
     *
    ...
    public static String[] splitSql(String sql) {
        ...
        List<JStringUtils.Token> tokens = JStringUtils.tokenizeSql(sql.toLowerCase());
        Stack<String> nestStack = new Stack<>();
        String lastPop = "";
        int start = 0;
        int cursor;
        boolean quote = false;
        ...
        for (cursor = start; cursor < tokens.size(); cursor++) {
            JStringUtils.Token token = tokens.get(cursor);
            if (quote) {
            ...
            } else if (nestStack.isEmpty() && token.text.equals(";")) {
